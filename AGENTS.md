...
The home screen is a centered brand mark (`public/logo.png`, also the favicon) with four actions: Today's Challenge, Free Play, Custom Game, and Stats. During a run the same mark sits at the top left of the viewport and returns to that home screen without wiping a saved Daily or Free Play attempt. Today's Challenge and Free Play start the draft immediately; Custom Game only asks for a seed. The team name is requested on the locked-in screen after five players and a coach are picked, before Start Major. Saved Daily and Free Play attempts resume immediately and keep their team name when one was already chosen.
...
- `npm run validate-data` must stay green. It enforces regime consistency, 24 revision-pinned Majors, each played field size, five unique starters, relational integrity, and that every referenced logo, player photo, bonus icon, and map-art file exists.
