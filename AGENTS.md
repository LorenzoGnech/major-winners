...
- Before each map the player picks a game plan (`standard`, `rush`, `late-execute`, `pick-heavy`, `executes`, `anti-strat`, `contact`, `risky`). Standard defaults is the no-op baseline and the default if the player confirms without changing it. Other plans shift fit-adjusted attributes plus anti-strat, economy discipline, pistol/early-round bias, and (for contact) starting morale. Risky plays push smokes and 50/50s (higher entry, lower consistency). The opponent is not planned. A BO3 pauses after each map until `startNextMap`; skip plays leftover maps on Standard.
