...
- Morale starts from chemistry and coach comeback resilience, decays toward 50 each round, and moves on clutches (`+8` / `−6`) and a player timeout (`+10` next round). Timeouts are player-called, one per map, never automatic, and apply to the immediate next round.
