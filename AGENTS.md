...
- Game plans (`standard`, `rush`, `late-execute`, `pick-heavy`, `executes`, `anti-strat`, `contact`, `risky`) still exist in the engine and `GamePlanPicker`, but the live UI does not show them. Tournament play uses Standard. Other plans shift fit-adjusted attributes plus anti-strat, economy discipline, pistol/early-round bias, and (for contact) starting morale. Risky plays push smokes and 50/50s (higher entry, lower consistency). The opponent is not planned. A BO3 starts the next shuffled map on Standard after the map-win overlay; skip plays leftover maps on Standard.
