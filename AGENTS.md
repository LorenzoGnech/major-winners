...
- All randomness flows through `src/engine/rng` (Mulberry32). `createRng` takes a uint32 or string (FNV-1a). `seedFromUtcDate` hashes the UTC `YYYY-MM-DD` day. A given seed must produce the same draft, coach order, and opponents. Match results match for the same seed, map queue, per-map game plans, and timeout timings.
...
Today's Challenge uses the UTC day and `seedFromUtcDate`, making the draft, coach order, and opponents deterministic for identical choices. Match results are deterministic for the same map queue, per-map game plans, and timeout timings. Restarting uses the same daily seed; Free Play remains random unless started as a Custom Game.
